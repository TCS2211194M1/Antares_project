...
class Domain extends Connection{
    ...
    function consultActivos(){
        ...
        INNER JOIN t_compra on t_workorder.t_workorder = t_compra.t_workorder WHERE t_workorder.ENTRY_STATUS='0'");
        return $ejec;
    }

    function consultInactivos(){
        $ejec = $this->execute("SELECT * FROM t_workorder INNER JOIN t_product on t_workorder.t_product = t_product.t_product 
         INNER JOIN t_compra on t_workorder.t_workorder = t_compra.t_workorder WHERE t_workorder.ENTRY_STATUS='1'");
        return $ejec;
    }
